 ...
 ******************************************************************************
  文 件 名   : epoll_server.c
  版 本 号   : 初稿
  作    者   : bseazeng
  生成日期   : Monday, May 2, 2016
  最近修改   :
  功能描述   : a tcp socket server with epoll
  函数列表   :
  修改历史   :
  1.日    期   : Monday, May 2, 2016
    作    者   : bseazeng
    修改内容   : 创建文件
        ...
        for (n = 0; n < nfds; ++n) 
		{
			...
			else 
			{
                ...
                if (ret < 1 && errno != 11) 
				{
		            ...
		            {
		             fprintf(stderr, "把 socket '%d' 从 epoll 删除失败！%s/n",events[n].data.fd, strerror(errno));
		            }
                    curfds--;
